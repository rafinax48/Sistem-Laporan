          ...
          <div className="grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-3">
            {practicums.map((p) => {
              ...
              return (
                <div
                  key={p.id}
                  className={`flex flex-col justify-between rounded-xl border p-5 transition-all bg-card shadow-[0_1px_2px_rgba(20,33,46,0.04)] ${
                    isSelected
                      ? "border-red ring-1 ring-red shadow-[0_2px_8px_rgba(192,57,43,0.12)]"
                      : "border-line hover:border-ink-soft hover:shadow-md"
                  }`}
                >
                  <div className="space-y-3">
                    <div className="flex items-center justify-between">
                      ...
                      <button
                        ...
                        className="font-mono text-[11px] text-red/80 hover:text-red hover:underline disabled:opacity-50"
                        title="Hapus praktikum ini beserta isinya"
                      >
                        {deletingId === p.id ? "Menghapus..." : "🗑️ Hapus"}
                      </button>
                    </div>

                    <Link href={`/practicums/${p.id}`} className="group block">
                      <h3 className="font-serif text-lg font-bold text-ink group-hover:text-red transition-colors line-clamp-2">
                        {p.name}
                      </h3>
                    </Link>
                    ...
                    <div className="grid grid-cols-2 gap-1.5 pt-2 font-mono text-[10px]">
                      <Link
                        href={`/practicums/${p.id}`}
                        className="rounded border border-line bg-hint/30 px-2 py-1 text-ink hover:border-red hover:text-red hover:bg-card transition-colors flex items-center gap-1"
                      >
                        <span>📊</span> Buku Nilai
                      </Link>
                      <Link
                        href={`/practicums/${p.id}/students`}
                        className="rounded border border-line bg-hint/30 px-2 py-1 text-ink hover:border-red hover:text-red hover:bg-card transition-colors flex items-center gap-1"
                      >
                        <span>👥</span> Praktikan
                      </Link>
                      <Link
                        href={`/practicums/${p.id}/assess`}
                        className="rounded border border-line bg-hint/30 px-2 py-1 text-ink hover:border-red hover:text-red hover:bg-card transition-colors flex items-center gap-1"
                      >
                        <span>📝</span> Nilai Laporan
                      </Link>
                      <Link
                        href={`/practicums/${p.id}/settings`}
                        className="rounded border border-line bg-hint/30 px-2 py-1 text-ink hover:border-red hover:text-red hover:bg-card transition-colors flex items-center gap-1"
                      >
                  ...
                  <div className="mt-5 flex items-center gap-2 border-t border-line pt-3 text-xs">
                    <Link
                      href={`/practicums/${p.id}`}
                      className="flex-1 text-center rounded-md bg-ink py-2 font-mono text-xs font-semibold text-white hover:bg-red transition-colors"
                    >
                      Buka Halaman Praktikum →
                    </Link>
                    <button
                      type="button"
                      onClick={() => handleOpenStudents(p)}
                      className="rounded-md border border-line px-2.5 py-2 font-mono text-xs text-ink-soft hover:text-ink hover:border-ink-soft transition-colors"
                      title="Buka panel cepat praktikan"
                    >
                      {isSelected ? "▲" : "▼"}
                    </button>
                  </div>
